# CircleCIでtiming dataに基づいたテスト分割をDartで利用できるようにするサンプルプロジェクト

## CircleCIのテスト結果

https://app.circleci.com/pipelines/github/operando/dart-test-CircleCI-timing-data/14/workflows/e4884496-b4b0-4d5b-af7a-0209097affac/jobs/16

## CircleCI Docs

- [Collecting test data](https://circleci.com/docs/2.0/collect-test-data/)
- [Running Tests in Parallel](https://circleci.com/docs/2.0/parallelism-faster-jobs/)
- [Test Insights](https://circleci.com/docs/2.0/insights-tests/)

## Other Links

- [Dart test - JSON Reporter Protocol](https://github.com/dart-lang/test/blob/master/pkgs/test/doc/json_reporter.md)
- [circleci tests split --split-by=timingsについて調べた・自作してみた](https://hoshinotsuyoshi.com/post/circleci_tests_split/)
- https://github.com/TOPdesk/dart-junitreport
- https://github.com/TOPdesk/dart-testreport
- https://github.com/operando/dart-junitreport
- https://github.com/operando/dart-testreport